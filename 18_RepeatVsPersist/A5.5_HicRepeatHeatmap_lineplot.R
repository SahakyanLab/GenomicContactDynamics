...
PreElmTissDyn.dir = paste0(wk.dir, "/out_HicRepeatHeatmapData/subfam_skewrep_atleast2sumrep") #, rep.group)
# hmclustPth = paste0(wk.dir, "/out_HicRepeatHeatmap/hm_famVssubfam_clust.csv")
out.dir = paste0(wk.dir, "/out_scratch") #paste0(wk.dir, "/out_HicRepeatHeatmap/subfam_skewrep_atleast2sumrep")
### OTHER SETTINGS #############################################################
# Age rank identifier
...
for(mx.nme in mx.nme.v){
  ...
  if(addLoess & mx.nme=="norm"){
    ...
    loe <- apply(X=mx, MARGIN=1, FUN=function(rw){
      
      if( any(!is.finite(rw)) ){
        return(NA)
      } else {
        
        cor.out <- doCorTest(xval=x.v, yval=rw, 
    ...
    perc.0.loe <- format(sum(is.na(loe))/length(loe)*100, digits=4)
    write(x=paste0(perc.0.loe, "% of ", length(loe), " rows cor not sig alpha 0.05"),
          file=paste0(out.dir, "/", id, "_", mx.nme, "_loess_heatmap.txt"))
