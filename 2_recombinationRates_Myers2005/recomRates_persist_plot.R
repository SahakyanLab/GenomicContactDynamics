...
#recombinationRates~persistent contact files
recom.persist.dir <- paste0(objective.dir, "/out_recomRates_persist")

#Create directories

#for output
#dir.create(paste0(objective.dir, "/out_recomRates_persist_boxplot"))
output.dir <- paste0(objective.dir, "/out_recomRates_persist_boxplot")  
...
#column name of ntis (number of tissues) of ij pairs in recombinationRates~persistent contact files
ntis        <- "NTISij"
...
foreach(gc=gc.v, .inorder=FALSE) %op% {
  ...
  foreach(chr=chr.v, .inorder=FALSE) %op% {
    ...
    #load data
    load(paste0(recom.persist.dir, "/chr", chr, "_min", gc, "Mb_recom_persist.RData"))
    ...
    foreach(rb=rb.v, .inorder=FALSE) %op% {
      ...
      #make recombinarate~persist plots (per chromosome)
      foreach(st=st.v, .inorder=FALSE) %op% {
        
        #png(filename=paste0(recom.persist.dir, "/", "bp_", rb, "_", st,"_chr", chr, "_RECOM_PERSIST_min", gc, "Mb_bin40Kb.png"),
        ...
        p <- ggplot(data=data, aes(x=data[,ntis], y=data[,st], group=data[,ntis])) +
          ...
          guides(fill=FALSE) + 
          stat_summary(fun.data=give.n, geom="text", size=2) + #use size 4 when using png (instead of ggsave)
          ...
          #                   breaks=unique(df[,"NTISij"]), labels=unique(df[,"NTISij"])) +
          scale_y_continuous(name=paste0(st, " RecombinationRate(cM/Mb)")) +
          theme.persist #from sourced "ggLayers_persist.R"
          
        if(drop.rows.NA!=0){
          p <- p + annotate("text", x=21, y=max(data[,st])+1.5, label=paste0("Dropped:", drop.rows.NA), size=3)
        } 
        
        if( st%in%c("MEANijP", "MEDIANijP", "SDEVijP", "MINijP", "MAXijP") ){
          
          pdf(file=paste0(output.dir, "/bp_min", gc, "Mb_", st, "_chr", chr, "_recom_persist.pdf"),
              height=8, width=8)
          ...
          ggsave(filename=paste0(output.dir, "/bp_min", gc, "Mb_", st, "_chr", chr, "_recom_persist.png"),
                 plot=p)
        } else {
          
          pdf(file=paste0(output.dir, "/bp_min", gc, "Mb_", rb, "_", st,"_chr", chr, "_recom_persist.png"),
              height=8, width=8)
          ...
          ggsave(filename=paste0(output.dir, "/bp_min", gc, "Mb_", rb, "_", st, "_chr", chr, "_recom_persist.png"),
                 plot=p)
        }
