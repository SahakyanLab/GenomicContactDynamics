...
################################################################################
pdf(file=paste0(out.dir, "/", src.id, "_boxplots.pdf"),
    width=35, height=10)
par(mfcol=c(4,7))
...
for(mut in mut.v){
  ...
  load(file=paste0(mutbin.dir, "/", mut.id, "_", src.id, "_mutCalcPerBin.RData"))
  ...
  for(chr in chr.v){
    ...
    # Assign bin to Cp (involved in at least 1 contact for that Cp)
    for(Cp in Cp.v){
    ...
    MUTCP.DF[[chr]][bin.mut,c("Tmut", "Nmsite", "TmutDIVNmsite",  "Nmsitenorm")] <- MUTBIN.DF[chr.TF, c("Tmut", "Nmsite", "TmutDIVNmsite",  "Nmsitenorm")]
  ...
  save(MUTCP.DF, file=paste0(out.dir, "/", mut.id, "_", src.id, "_mutCalcPerCp.RData"))
