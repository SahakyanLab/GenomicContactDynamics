...
### FUNCTION ###################################################################
generateMUTBINDF <- function(src.dir, data.id, src.id, out.dir, basecont.dir, 
  ...
  # Mutation type
  if(mut=="All"){
    mut.TF <- rep(T, times=length(ncv.df[,1]))
  ...
  incl.TF <- ncv.df$location%in%loc & ncv.df$ID_SAMPLE%in%sample.v & mut.TF
  ...
  # Return metadata
  meta <- c(mut.id, SIG.id, loc.id, sigEpLim.id, Nmut=sum(MUTBIN.DF$Tmut), Nsamp=samp.len, 
            meanNsamp=Nsamp.mean, medianNsamp=Nsamp.med, sample=samp.str)
  return(meta)
