...
SIG.v = c("RefSig.MMR1_RefSig.MMR2", colnames(sigExposure.df)[colnames(sigExposure.df)!="alt.ID"])

# Only consider samples/donor within this signature exposure percentage range (right closed interval).
sigEpLim.v = c("nosampfilter", "-1_0", "30_100", "50_100") 
#"nosampfilter" - skip filtering based on signature exposure, use all samples in the dataset 

nCPU = 2L # Number of combinations of mut.v, SIG.v, sigEpLim.v and loc.v
################################################################################
...
# Generate combinations of mut.v, sig.v, sigEpLim.v, loc.v
comb.mx <- data.matrix(expand.grid(1:length(mut.v), 1:length(SIG.v), 1:length(sigEpLim.v), 
                                   1:length(loc.v), stringsAsFactors=F))

toExport <- c("comb.mx", "mut.v", "SIG.v", "sigEpLim.v", "loc.v", "basecont.id.v",
              "basecont.affix.v", "src.dir", "data.id", "src.id", "out.dir", 
              "BASECONT.dir", "bin.len", "chrlen.df", "sigExposure.df")
i.len <- nrow(comb.mx)

#### PARALLEL EXECUTION #########
metadata <- foreach(itr=isplitVector(1:i.len, chunks=nCPU), .inorder=T, 
                    .combine="rbind.data.frame", .export=toExport, 
  ...
  chunk <- sapply(X=itr, simplify=FALSE, FUN=function(i){
    ...
    run.id <- paste0(data.id, "_", src.id, "_", mut, "_", SIG, "_",
                     loc, "_sigEperclimits_", sigEpLim)
    print(paste0("RUN: ", run.id), quote=F)
    
    return(
      generateMUTBINDF(src.dir=src.dir, data.id=data.id, src.id=src.id, out.dir=out.dir, 
...
write.csv(x=metadata, file=paste0(out.dir, "/", data.id, "_", src.id, "_", metadata.id, ".csv"),
          row.names=F)
