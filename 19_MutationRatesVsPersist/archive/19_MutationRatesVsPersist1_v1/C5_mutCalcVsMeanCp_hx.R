...
out.dir = paste0(wk.dir, "/out_mutCalcVsMeanCp_hx")
### OTHER SETTINGS #############################################################
gcb = "min2Mb"
...
for(mut in mut.v){
  ...
  # MUTCP.DF
  load(file=paste0(mutCp.dir, "/", data.id, "_", mut.id, "_", src.id, "_mutCalcPerCp.RData"))
  ...
  for(calc in calc.v){
    ...
    hxbinCp0[[paste0(mut, ".", calc)]] <- makeHexbinggplot(xvar=MUTCP.DF[,"wmeanCp0"],
                                                           ...
                                                           xlab="wmeanCp0",
                                                           ylab=calc,
                                                           title=paste0(mut.id, "_", src.id, "_", "_meanCpFrom", meanCp.id, "_", calc, "_hxbincuts", hxbincuts),
...
ggexport(p.arr, height=mut.v.len*10, width=calc.v.len*10,
         filename=paste0(out.dir, "/", data.id, "_", src.id, "_meanCpFrom", meanCp.id, "_calcXwmeanCp0.pdf"))
rm(hxbinCp0)

p.arr <- ggarrange(plotlist=hxbinCp, nrow=mut.v.len, ncol=calc.v.len, legend=NULL)
ggexport(p.arr, height=mut.v.len*10, width=calc.v.len*10,
         filename=paste0(out.dir, "/", src.id, "_meanCpFrom", meanCp.id, "_calcXwmeanCp.pdf"))
