...
### OTHER SETTINGS #############################################################
bin.len = 40000
chr = "CHRREPLACE" #paste0("chr", c(1:22, "X"))
gcb = "min2Mb"
...
#for(chr in chr.v){
  ...
  PAIRCP.MX <- foreach(pair.ind.v=isplitVector(1:pair.len, chunks=nCPU), 
    ...
    chunk <- sapply(X=pair.ind.v, simplify=F, FUN=function(pair.ind){
      ...
      # Won't work for genes longer than min.gap (but only 7 out of 24910 longest tr
      # and from different chromosomes)
      #> gene.v[txlen > 2e6]
      #[1] "NBPF20"  "CNTNAP2" "CSMD1"   "PTPRD"   "DMD"     "DLG2"    "MACROD2"
      #     chr1      chr7      chr8      chr9      chrX      chr11     chr20
