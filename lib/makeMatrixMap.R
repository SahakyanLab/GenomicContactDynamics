...
makeMatrixMap <- function(df.lst = 'list of 1 (upper) or 2(upper and lower) data.frame(x, y, value)', 
                          ...
                          species.id = "e.g. osa", transform.fun = 'list of functions for
                          transforming matrix values, in order of map type'
                          ){
  ...
  for(m in 1:metric.v.len){
    ...
    if( !is.null(transform.fun[[m]]) ){
      df.lst[[m]]$value <- sapply(df.lst[[m]]$value, FUN=transform.fun[[m]])
    }
