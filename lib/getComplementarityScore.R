...
getComplementarityScore <- function(
  ...
  #incl.ij <- !is.na(contact.mx[,1])
  #contact.mx <- contact.mx[incl.ij,]
  ...
  if(type=="align"){
    ...
    if( length(SCORE)!=length(contact.mx[,1]) ){
      stop("Checkpoint!")
    } else {
      return(SCORE)
    }
  ...
  if( length(SCORE)!=length(contact.mx[,1]) ){
    ...
    #SCORE.FINAL <- rep(NA_integer_, times=length(incl.ij))
    #SCORE.FINAL[incl.ij] <- SCORE
    #return(SCORE.FINAL)
    return(SCORE)
  }
