...
df.up <- df.up[df.up$i<df.up$j,]
df.up$value <- 1:10

# Method 1 (directly fills upper triangle)
df.1 <- df.up[order(df.up$j, df.up$i, decreasing=F),]
MX <- matrix(data=NA, nrow=5, ncol=5)
MX[ upper.tri(MX, diag=F) ] <- df.1$value

# Method 2 (fills lower triangle -> transpose to get upper triangle only)
df.2 <- df.up[order(df.up$i, df.up$j, decreasing=F),]
