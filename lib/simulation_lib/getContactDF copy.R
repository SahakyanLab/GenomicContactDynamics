...
getContactDF <- function(metric.dir=metric.dir, metric="Cp", gcb = "min2Mb", 
  ...
  if( metric%in%c("Cs.raw", "Cs.norm", "Cp") ){
    ...
    type.id <- unname(m.v["type"])
    if( unname(m.v["type"])=="Gfree"){
      type.id <- "kmer"
    }
    ...
    if( unname(m.v["value"])=="disc" & unname(m.v["type"])%in%c("kmer", "align") ){
      value <- "group"
    } else if( unname(m.v["value"])=="cont" ){
      ...
      stop("getContactDF(): Invalid type in metric argument for c|| values.")
    }
    
    # Take upper triangle
    #df <- cbind.data.frame(CII.MX[,c("i", "j")], value=CII.MX[,value])
    df <- as.data.frame(CII.MX[,!dimnames(CII.MX)[[2]]%in%c("Cp", "sdDifference")])
    data.table::setnames(x=df, old=value, new="value", skip_absent=FALSE)
    rm(CII.MX, m.v, value)
