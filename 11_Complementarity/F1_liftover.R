################################################################################
# Liftover CII.MX hg38 contacts to hg19
################################################################################
...
src.dir = paste0(wk.dir, "/out_constraints_hg38_GfreeSingleNorm/merged_final") # 0-based
...
foreach(itr=isplitVector(1:chrs.len, chunks=nCPU), .inorder=F
  ...
  for(i in itr){
    ...
    load(file=paste0(src.dir, "/", chr, "_", src.id, ".RData"))
    df <- cbind(start.i=rep(NA), end.i=CII.MX[,"i"] * src.bin.len,
                start.j=rep(NA), end.j=CII.MX[,"j"] * src.bin.len)
    df[,"start.i"] <- df[,"end.i"] - src.bin.len 
    df[,"start.j"] <- df[,"end.j"] - src.bin.len
    
    # Limit coordinates to chromosome length
    tot.len <- chrlen.df$length.bp[chrlen.df$chromosome == chr]
    df[df > tot.len] <- tot.len
    rm(CII.MX, tot.len)
    
    # Complete df
    df <- data.frame(chr=rep(chr), df)
