...
plotFeatDensPerRadWindow <- function(
  ...
  count.v = c("raw", "norm"),
  # Pick hist.breaks.res that will not generate 0 densities
  # Should cover all the rVal and may vary between Chrom3D structure
  # hist.breaks <- seq(from=0, to=max(RPCOUNT.DF$rVal), by=hist.breaks.res)
  hist.breaks.res = 0.5,
  ...
  if(regenerateCountData==TRUE){
    ...
    # Load DOMXYZRFEAT.DF
    file.nme <- load(fileName=DOMXYZRFEATfile)
    eval(parse(text=paste0(
    ...
    print(paste0("Periphery: ", periphery, "; ", nrow(DTA), " beads."), quote=FALSE)
    ...
    #} else if(dividingR=="BIN"){
    #  rVal <- BINorSLIDE(numVec=DTA$radDist, action="BIN", 
    #                     numPoints=NULL, dr=dr)
    #} else if(dividingR=="NONE"){
    #  rVal <- unique(DTA$radDist)
    } else {
      stop("Invalid input for dividingR.")
    }
  ...
  hist.breaks <- c(seq(from=0, to=max(RPCOUNT.DF$rVal), by=hist.breaks.res), 
                   max(RPCOUNT.DF$rVal))
  ...
  for(cnt in count.v){
    ...
    ggplot( data=df, aes(x=mid, y=value, group=variable) ) +
      ...
      geom_point(size=1.5, aes(colour=factor(variable))) + 
