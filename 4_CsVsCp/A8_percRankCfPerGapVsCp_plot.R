################################################################################
# Plot percentile rank Cf at certain gap vs. Cp.
################################################################################
...
src.dir = paste0(wk.dir, "/out_percRankCfPerGap")
out.dir = paste0(wk.dir, "/out_percRankCfPerGapVsCp_plot")
### OTHER SETTINGS #############################################################
gcb = "min2Mb"
...
for(ct in cts){
  ...
  MX <- foreach(itr=isplitVector(1:chrs.len, chunks=nCPU), 
    ...
    chunk <- sapply(X=itr, simplify=F, FUN=function(i){
      ...
      chr <- chrs[[i]]
      load(paste0(src.dir, "/", ct, "_", chr, "_percRankCfPerGap_", gcb, ".RData"))
      mx <- mx[,dimnames(mx)[[2]] != "obs.val"] 
  ...
  # Filter 
  df <- df[!is.nan(df$value),] # obs / exp of NaN means exp.val of 0 so no contact at gap
  df <- df[df$value != 0,] # Contact not in tissue
  df <- df[ df$gap >= gap.rng.closed[[1]] & df$gap <= gap.rng.closed[[2]], ]
  
  df$Cp <- factor(df$Cp, levels=as.character(Cps))
 
  P.LST[[ct]] <- ggplot(data=df, aes(x=Cp, y=value)) +
