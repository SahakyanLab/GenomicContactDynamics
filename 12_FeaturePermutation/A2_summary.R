...
permt.dir = paste0(wk.dir,"/out_associate_runB_cp21_b1b2_comOlap")
out.dir = paste0(wk.dir,"/out_summary")
foi.dir = paste0(data.dir, "/funx_data_fixCoordSys/masterpool_hg19_convTo1based/reduced_b1b2")
### OTHER SETTINGS #############################################################
foifile.v = paste0(wk.dir, "/foifile/foifile_test1")
out.name.v = "b1b2"#"ABscomp"
gcb = "min2Mb"
Cp.v = 21
...
BIN.MX <- sapply(X=paste("chr", c(1:22, "X"), sep=""), simplify=FALSE, 
                 FUN=function(chr){
...
foifile.v.len <- ifelse(is.null(foifile.v), 1, length(foifile.v))
for(f in 1:foifile.v.len){
  
  foifile <- foifile.v[f]
  out.name <- out.name.v[f]
  ...
  ct.v.uniq <- unique(ct.v)
  
  # Calculate length of contacting regions (cell-type specific); to be divided to comOlap
  totlen.bin.v <- sapply(X=ct.v.uniq, simplify=TRUE, FUN=function(ct){
    totlen.bin <- totlen.bin0
  ...
  if(plotOnly==FALSE){
    ...
    for(i in 1:foi.v.len){
      ...
      for(x in evalf.lab){
        PERMTSUM$pval[foi,x] <- as.numeric( PERMT[[x]][["pval"]] )
        PERMTSUM$obs[foi,x]  <- as.numeric( PERMT[[x]][["observed"]] )
        PERMTSUM$alt[foi,x]  <- PERMT[[x]][["alternative"]] 
      }
      rm(PERMT)
  ...
  #---------------------------------------
  foi.v <- gsub(x=foi.v, pattern="ct\\_|foi\\_|desc\\_|\\.bed", replacement="")
  #p.lst <- list()
  for(i in 1:length(evalf.lab)){
    ...
    if(evalf.nme[i]=="comOlap"){
      df$obs <- 100*df$obs/totlen.bin.v[ct.v[i]]
  ...
  #p.arr <- ggarrange(plotlist=p.lst, nrow=1, ncol=1)
  #ggexport(p.arr, height=50, width=50,
  #         filename=paste0(out.dir, "/", out.id, "_", out.name, "_permplot.pdf"))
