...
### OTHER SETTINGS #############################################################
permtsum.id = "nperm10000_seed429"
eval.f.v = c("comOlap")
pval.cutoff = 0.05
plotOnly = FALSE
################################################################################
...
if(plotOnly==FALSE){
  ...
  permtsum.v = c("Cp21", "CptopCP3", "CpAllCsmatch")
...
v <- brewer.pal(name="PRGn", n=11)[c(3,9)]
...
heatmap.2(x=HMAP.MX, Rowv=TRUE, Colv=TRUE,
          ...
          breaks=c(-1,-0.5,0,0.1,1), col=c(v[1], "gray90", "gray70", v[2]), 
