...
ct.v <- c("hg19;hg19", "hg19;hg19",
          "hg19;FC", "hg19;FC")
metric.v <- c("CII.cont.kmer.5;Cp", "CII.disc.kmer.5;Cp", 
              "CII.cont.kmer.5;Cs.norm", "CII.disc.kmer.5;Cs.norm")

ct.v <- c("FC", "FC")
metric.v <- c("Cs.norm", "Cp")

ct.v <- c("hg19;FC")
metric.v <- c("Cp;Cs.norm")
...
mark.x = c(485,494,1860,1869) #1965:1974 #1915:1924 #3090:3099 #1725:1734  
mark.y = c(1915,1924,1965,1974) #485:494 #1860:1869 #630:639 #1675:1684 

# Output specifications

# Useful in case not whole chr is to be plotted
out.id = "chr17_whole_50ToInf_FC_CpCs" #"chr17_50ToInf_x1915To1924_y1860To1869_gene1_FC_CpCs" #"chr1_whole_50ToInf_FC_CpCIICs_confimation" #"genes1" #"SGIP1" #"KRAS" #"whole_maskMidSquare_gap50up_maskx3038To6232y1To3565"
# If scalebr.v==NULL, no scale bar
scalebr.v = NULL #c(xmin=1, xmax=100, ymin=6183, ymax=6232)
res = 300
...
if( any(duplicated(map.id.v)) ){
  warning("Duplicated row in map.id.v.")
}
...
cowplot::save_plot(p.arr, filename=paste0(out.dir, "/", out.name0, "_contProb", contProb, ".png"), 
                   base_height=out.dim[1]*10, base_width=out.dim[2]*10, limitsize=F)
