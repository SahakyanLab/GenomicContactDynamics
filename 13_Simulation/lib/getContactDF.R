...
getContactDF <- function(metric.dir=metric.dir, metric="Cp", gcb = "min2Mb", 
  ...
  if( metric%in%c("Cs.raw", "Cs.norm", "Cp") ){
    ...
    MELT.MX$rest <- MELT.MX$lower.tri <- MELT.MX$lower.tri.nocontact <- NULL
    ...
    if(metric=="Cp"){
      ...
      if(ct%in%ct.v){
        ...
        stop("Invalid ct for Cp-based metric.")
      }
      ...
      if(!ct%in%ct.v){
        stop("Invalid ct for Cs-based metric.")
      }
  ...
  print(paste0(metric, " values retrieved!"), quote=FALSE)
