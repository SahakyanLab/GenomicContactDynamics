...
#dir.create(paste0(objective.dir, "/out_AnnotationVsPersist"))
...
anno.file <- fread(file=paste0(annoFile.path, "/", anno.filename), 
...
foreach(gc=gc.v, .inorder=FALSE) %op% {
  ...
  ANNO.PERSIST.MX <- foreach(chr=chr.v, .inorder=FALSE, .combine="rbind") %op% {
    ...
    col345 <- foreach(itr=as.vector(col12[, "HiC21bin"]), .combine="rbind" ) %op% {
      ...
      ind.all      <- unique(c(ind.i, ind.j))  
      hits.NA      <- apply(persist.mx[ind.all,c(i, j)], MARGIN=c(1,2), 
                            function(x) ifelse(x==itr, x<-NA, x<-x))
