#The HapMap recombination map (Myers et al. 2005) was downloaded 
#from the HapMap website (2011-01_phaseII_B37) and lifted to GRCh35 to GRCh37 by Munch et al., 2014
#untar(tarfile="/Users/ltamon/ProjectBoard/HiC_contacts_dim/RecombinationRates/recombinationRates_data/genetic_map_HapMapII_GRCh37.tar.gz", 
#list=FALSE, exdir="/Users/ltamon/ProjectBoard/HiC_contacts_dim/RecombinationRates/recombinationRates_data/recomRates_2011-01_phaseII_B37_Myers")
#list=FALSE means it will extract the data, if TRUE it will just return a list of files
...
#for testing make sure to include at least 500 of ij contacts in PERSIST.MX
#to make sure that there will be ij contacts both having rates for all chromosomes
#i still have to find a way to do next in foreach to check for this
#if there are no ij contacts both having recombination rates
#if(length(ind.and)==0) {
...
objective.dir  <- "/Users/ltamon/ProjectBoard/HiC_contacts_dim/recombinationRates/recombinationRates_Myers2005"
#recombination rates data (Myers et al., 2005; Munch et al., 2014)
recomRates.dir <- paste0(objective.dir, "/recomRates_2011-01_phaseII_B37_Myers")
#data structure of recombination rates data (2011-01_phaseII_B37)
data.str.path  <- paste0(objective.dir, "/recomRates_2011-01_phaseII_B37_Myers/data_structure.txt")
#HiC_Human21 persistent contact files
persist.dir    <- "/Users/ltamon/Database/HiC_features_GSE87112_RAWpc"

#Create directories 

#for output, the assignment of recombination rates to persistent contacts
#dir.create(paste0(objective.dir, "/out_recomRates_persist"))
output.dir <- paste0(objective.dir, "/out_recomRates_persist")  
...
############################################################################################################ 
############################################################################################################ 
#PART1 - Objective: assign recombination rates to 40-Kb bins

#nested foreach to get data for each chromosome for each gap between contacts
foreach(gc=gc.v, .inorder=FALSE) %op% {
  ...
  foreach(chr=chr.v, .inorder=FALSE) %op% {
    #read recombination rate file for the current chromosome and gap.contact
    recomRates.mx <- fread(file=paste0(recomRates.dir, "/genetic_map_GRCh37_chr", chr,".txt"), header=T, data.table=F)
  ...
  #} foreach closed to check if recombination rate files have missing values, all files - no NA
    ...
    #load persistent contact files derived from HiC data from 21 human tissues 
    ...
    #alternatively, one can just divide the chromosome into 40-KB bins, i have a file with the lengths
    #of human chromosomes
    bins.uniq  <- unique(unlist(persist.mx[,c(i, j)]))
    ...
    olap.mx <- WhichOverlap(start.query=recomRates.mx[,"POSbp"], end.query=recomRates.mx[,"POSbp"], space.query=rep("a", nrow(recomRates.mx)),
    ...
    #nrow(olap.mx)==nrow(rates.bin)
    rates.bin      <- cbind(binAssignment=bins.uniq[olap.mx[,"subject"]], recomRates.mx[olap.mx[,"query"],])
    count.indv.r.b <- by(rates.bin[,"RATEcMperMb"], rates.bin[,"binAssignment"], function(x) length(x))
    mean.r.b       <- by(rates.bin[,"RATEcMperMb"], rates.bin[,"binAssignment"], function(x) mean(x))
    median.r.b     <- by(rates.bin[,"RATEcMperMb"], rates.bin[,"binAssignment"], function(x) median(x))
    #sd can give NA values when n=1
    sd.r.b         <- by(rates.bin[,"RATEcMperMb"], rates.bin[,"binAssignment"], function(x) sd(x))
    indv.pos.b     <- by(rates.bin[,"POSbp"], rates.bin[,"binAssignment"], function(x) paste(x, collapse = ","))
    indv.r.b       <- by(rates.bin[,"RATEcMperMb"], rates.bin[,"binAssignment"], function(x) paste(x, collapse = ","))
    indv.map.b     <- by(rates.bin[,"MAPcM"], rates.bin[,"binAssignment"], function(x) paste(x, collapse = ","))
    
    RECOM.BIN.MX <- cbind(Chr=rep(paste0("chr", chr), length(indv.pos.b)), Bin=unique(rates.bin[,"binAssignment"]), 
                          MEANrb=mean.r.b, MEDIANrb=median.r.b, SDEVrb=sd.r.b, COUNTINDVrb=count.indv.r.b,
                          INDVRATEScMperMb=indv.r.b, INDVPOSbp=indv.pos.b, INDVMAPcM=indv.map.b) #rownames refer to bin
    row.names(RECOM.BIN.MX) <- NULL
    
#-------------------------------------------------------
    
#PART2 - Objective: make a big matrix with the ij contacts, recombination rates and relevant statistical values
   
    #only isolate ij contacts (from persist.mx) that have recombination rate values for both i and j (see RECOM.BIN.MX)
    bin.num  <- as.numeric(RECOM.BIN.MX[,"Bin"])
    ind.and  <- which(persist.mx[,i]%in%bin.num & persist.mx[,j]%in%bin.num)
    #what to do if there are no ij contacts both having recombination rates
    ntis.and <- persist.ntis[ind.and] #ntis index in PERSIST.MX correspond to PERSIST.MX$hits index
    ...
    #subject <- bin from RECOM.BIN.MX (stored as bin.num variable), which has recombination data (etc.) for a particular bin
    olap.i.and <- WhichOverlap(start.query=i.and, end.query=i.and, space.query=rep("a", length(i.and)),
                               start.subject=bin.num, end.subject=bin.num, space.subject=rep("a", length(bin.num)),
                               maxgap=-1L, minoverlap=1L)
    olap.j.and <- WhichOverlap(start.query=j.and, end.query=j.and, space.query=rep("a", length(j.and)),
                               start.subject=bin.num, end.subject=bin.num, space.subject=rep("a", length(bin.num)),
                               maxgap=-1L, minoverlap=1L)
    ...
    #can add MAPcM values in the future
    mx1.list <- list()
    ...
    save(RECOM.PERSIST, file=paste0(output.dir, "/chr", chr, "_min", gc, "Mb_recom_persist.RData"))
    #RECOM.PERSIST.MX=result
    
  } #end bracket for chromosome foreach
  
  allresult <- lapply(list(MEANrb=allMEANrb, MEDIANrb=allMEDIANrb), function(x){
    x <- do.call(rbind, x)
    x <- as.data.frame(x)
  })
  
  allstat   <- lapply(list(RATESstat=allrates.stat, CONTACTSstat=allij.stat), function(x) do.call(rbind, x))
  ...
  save(RECOM.PERSIST, file=paste0(output.dir, "/chrALL_min", gc, "Mb_recom_persist.RData"))
