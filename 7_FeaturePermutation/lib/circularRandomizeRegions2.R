...
circularRandomizeRegions2 <- function(A, genome="hg19", mask=NULL, max.mask.overlap=NULL, max.retries=10, verbose=TRUE, ...) {
  ...
  getRandomChr <- function(chr, gam) {
    ...
    if(!is.null(max.mask.overlap)) {
      ...
      num.ov <- sum( width(commonRegions(toGRanges(spin.A), toGRanges(gam$mask))) )
      den <- sum( width(toGRanges(spin.A)) )
      
      num.retries <- 0
      #while(num.ov/nrow(spin.A) > max.mask.overlap & num.retries < max.retries) {
      while(num.ov/den > max.mask.overlap & num.retries < max.retries) {
        if(verbose==TRUE) {
          ...
          message(paste0("Chromosome ", chr, ". Too much overlap with the mask: Num. Regions: ", den, "  Num. Overlaps: ", num.ov, "  Pct. Overlap: ", (num.ov/den)*100, ". Retrying..."))  
        }
        spin <- floor(runif(1)*chr.len)
        spin.A <- spinChromosome(chr.A, spin, chr.len)
        num.ov <- sum( width(commonRegions(toGRanges(spin.A), toGRanges(gam$mask))) )
