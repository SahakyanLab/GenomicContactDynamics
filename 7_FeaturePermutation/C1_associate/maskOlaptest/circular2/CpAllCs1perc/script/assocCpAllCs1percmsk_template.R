...
out.dir = paste0(wk.dir,"/out_associate_masktest")
### OTHER SETTINGS #############################################################
gcb = "min2Mb"
...
for(i in 1:i.len){
  ...
  if(bgr=="HiCAll"){
    ...
    mask.bed <- reduce(union(mask.bed, mask.bed1))
    rm(mask.bed1); gc()
    if( !supplyMask & length(commonRegions(toGRanges(HiCAll), mask.bed))>0 ){
