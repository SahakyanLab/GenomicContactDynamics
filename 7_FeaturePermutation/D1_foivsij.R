...
tmp <- rowSums(FEATVSCP.MX[,-1])
if( any(tmp!=FEATVSCP.MX[,1]) ){ stop("Checkpoint 1.") } 
rm(tmp)
v <- colSums(FEATVSCP.MX)
if( v["All"]!=sum(v[-1]) ){ stop("Checkpoint 2.") } 
...
pair.len <- length(levels(df$Pair))
if(pair.len>10){
  
  coul <- unname(c(pal_npg("nrc")(10), 
