...
rawbmx.dir = paste0(wk.dir, "/binmx/out_bindata_1perc_HiCNorm")
normbmx.dir = paste0(wk.dir, "/binmx/out_bindata_1perc_HiCNorm_deva")
### OTHER SETTINGS #############################################################
...
for(chr in chr.v){
  ...
  if( !identical(rw,nrm) ){
    print(paste0(chr, " :BIN.MX not identical."), quote=FALSE)
  } else {
    print(paste0(chr, " :BIN.MX identical."), quote=FALSE)
  }
  ...
  if( !identical(rw,nrm) ){
    print(paste0(chr, " :BIN.MX not consistent after changing cut-off."), quote=FALSE)
    next
  }
