...
do_subsample = True  # Also set to true to load subsampled data
fit_model = True
...
sampling_id = '0point1percExceptgrthn18'  # '5percExceptgrthn18'
# percs = np.repeat([5,100], [18,3])
...
def concat_chrcsv(csv_dir, gcb, chrs):
    ...
    df = []
    [df.append(pd.read_csv(csv_dir + '/' + gcb + '_' + chr + '_MLtbl.csv'))
     for chr in chrs]
    df = pd.concat(df)
...
def choose_features(df, feat_regex, label_col):
    ...
    is_chosen = [bool(re.search(pattern=final_regex, string=feat))
...
def sampleGroupInSeries(dfSeries, perc, group, seed_val):
    ...
    row_ind_group_sampled = list(
        np.random.choice(a=row_ind_group, size=samp_size))
...
def sampleManyGroupsInSeries(dfSeries, percs, groups, seed_val):
    ...
    seed_generated = np.random.randint(
        low=0, high=1000, size=num_groups, dtype=int)
    ...
    for i in range(num_groups):
        ...
        row_ind_manygroups_sampled = row_ind_manygroups_sampled + \
            sampleGroupInSeries(dfSeries, perc, group, seed_val=seed_i)
...
if do_subsample:
    csv_id = csv_id + '_' + feat_regex_id + \
        '_seed' + str(seed_val) + '_' + sampling_id
...
if generate_train_data_subsample:
    ...
    if do_subsample:
        ind = sampleManyGroupsInSeries(
            train_data['LABEL'], percs, groups, seed_val=seed_val)
...
if fit_model:
    ...
    model_id = gcb + '_' + chrs_id + '_' + feat_regex_id + '_' + \
        presets + '_' + str(time_limit) + 'sec_' + problem_type
    ...
    if do_subsample:
        ...
        model_id = model_id + '_subsample_seed' + \
            str(seed_val) + '_' + sampling_id
...
if fit_model:
    predictor = TabularPredictor(label=label_col, eval_metric=metric, path=model_path, problem_type=problem_type, learner_kwargs={
                                 'label_count_threshold': label_count_threshold}).fit(train_data, ag_args_fit={'num_gpus': num_gpus}, presets=presets, time_limit=time_limit)
